...
# -*- coding: utf-8 -*-
"""
M4-B2：其他 L1 错误（跨类 tilt）规则可纠偏样本写回 v3。

用法：
  python3 performance_evaluation/eval_p1_subset.py --compare \
    --export-other-fixable auto --export-regression-fixable auto
  python3 performance_evaluation/patch_p1_other_l1_v3.py \
    --csv performance_evaluation/exports/p1_other_fixable_*.csv
  python3 performance_evaluation/patch_p1_regression_v3.py \
    --csv performance_evaluation/exports/p1_regression_fixable_*.csv --write
  python3 performance_evaluation/patch_p1_other_l1_v3.py --csv ... --write
...
from eval_p1_subset import P1_PRIMARY_HUMAN, P1_PRIMARY_MODEL, P1_REGRESSION_HUMAN, P1_REGRESSION_MODEL, replay_rules  # noqa: E402
...
def _is_other_l1_error(human_l1: str, model_l1: str) -> bool:
    if human_l1 == model_l1:
        return False
    if human_l1 == P1_PRIMARY_HUMAN and model_l1 in P1_PRIMARY_MODEL:
        return False
    if human_l1 in P1_REGRESSION_HUMAN and model_l1 == P1_REGRESSION_MODEL:
        return False
    return True


def main() -> int:
    parser = argparse.ArgumentParser(description="其他 L1 跨类纠偏写回 v3")
    parser.add_argument("--db", type=Path, default=DEFAULT_DB)
    parser.add_argument(
        "--csv",
        nargs="+",
        required=True,
        help="export-other-fixable CSV；可 glob（shell 展开多个时取最新）",
    )
    ...
    fixable = [
        r
        for r in enriched
        if _is_other_l1_error(r["human_l1"], r["model_l1"])
    ...
    for r in fixable:
        ...
        meta["l1"] = r["replay_l1"]
        meta["l2"] = r["replay_l2"]
        meta["patch_p1_other_l1_at"] = applied_at
    ...
    out = STATE_DIR / f"patch_p1_other_l1_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
