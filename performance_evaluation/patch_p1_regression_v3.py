...
# -*- coding: utf-8 -*-
"""
M4-A：将 P1 回归「规则可拉回」样本写回 v3（人工=业务，v3=非问题，replay→业务）。

与 patch_p1_fixable_v3.py 方向相反；仅 UPDATE 明确 fixable 行。

用法：
  python3 performance_evaluation/eval_p1_subset.py --compare --export-regression-fixable auto
  python3 performance_evaluation/patch_p1_regression_v3.py --csv performance_evaluation/exports/p1_regression_fixable_*.csv
  python3 performance_evaluation/patch_p1_regression_v3.py --csv ... --write
...
def main() -> int:
    parser = argparse.ArgumentParser(description="P1 回归 fixable 样本安全写回 v3（M4-A）")
    parser.add_argument("--db", type=Path, default=DEFAULT_DB)
    parser.add_argument(
        "--csv",
        nargs="+",
        required=True,
        help="export-regression-fixable CSV；可 glob（shell 展开多个时取最新）",
    )
    ...
    fixable = [
        r
        for r in enriched
        if r["human_l1"] in P1_REGRESSION_HUMAN
        and r["model_l1"] == P1_REGRESSION_MODEL
    ...
    for r in fixable:
        print(
            f"  {r['opinion_id']}  {r['model_l1']} -> {r['replay_l1']}/{r['replay_l2']}  "
            f"{(r['text'] or '')[:50]}"
        )
    ...
    for r in fixable:
        ...
        meta["l1"] = r["replay_l1"]
        meta["l2"] = r["replay_l2"]
        meta.pop("positive_capture", None)
        meta["patch_p1_regression_at"] = applied_at
    ...
    out = STATE_DIR / f"patch_p1_regression_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
