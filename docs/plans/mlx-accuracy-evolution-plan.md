...
> **创建日期**：2026-07-08  
> **最后更新**：2026-07-15  
> **状态**：Ollama 测试运营中（R3）；MLX 重训暂停  
> **关联**：`performance_evaluation/state/evolution_loop_mvp_plan_20260604.md`、`docs/VOC_V1.5_开发记录.md` R22–R26
...
| 2026-07-09 | holdout 初始化完成；baseline 89.74%；MLX v2/v3/v4 smoke 均 2/5 |
| 2026-07-13 | **切 Ollama 生产**；R1 fixable 14；R2 patch 写库 14 条 |
| 2026-07-13 | 全库 L1 **86.03%** L2 **78.25%**（+1253 新复核） |
| 2026-07-15 | **Ollama ~200 条/天** 测试运营；R3 准确率 **待 07-16 更新** |
| 2026-07-15 | 记录优化待办 O1–O3（见下） |
| 2026-07-16 | 记录 **O4** 车端充电 vs LFC 边界；近 4 天 Ollama 投喂待评估 |

---

# Ollama 测试运营优化待办（2026-07-15 记录）

> 测试运营中发现的问题，供后续规则/特征/prompt 迭代；**准确率批次数据待明天更新**。

| ID | 问题 | 业务口径 | 建议优化方向 | 优先级 |
|----|------|----------|--------------|--------|
| **O1** | 爆胎、事故、保养协助等被标为「服务类/售后」 | 属协助/咨询请求，**非产品故障、非服务投诉** → L1 倾向 **非问题** | 规则：爆胎/扎钉/道路救援/移动服务车/保养协助；硬负样本；与 R2 主靶 9 条同类 | P0 |
| **O2** | 车型信息未利用 | VIN 前缀：**LJU=电车**，**SCC=Emira** | 入库解析 VIN → 车型 hint 注入 `classify_text` prompt/规则 | P1 |
| **O3** | APP 来源用车分享/游记叙事 | 无明确投诉/故障 → **非问题** | 来源=APP + 叙事特征 guard；few-shot 样例 | P1 |
| **O4** | 「车端充电问题」L2 **频繁乱用** | **车端充电** 仅用于：充电功率低、预约后不充电等**车端充电能力/行为**问题 | L2 映射收紧；充电相关 guard 分流 | P0 |
| | 国内充电桩下线、蔚来/浩瀚等**第三方桩商抱怨** | 属 **LFC 问题**（充换电设施），非车端充电 | 关键词：桩下线、蔚来、浩瀚、闪充站、第三方桩 → LFC；`patch_v3_l2_replay` / l2 replay | P0 |

### 当前执行（与待办并行）

| 步骤 | 状态 |
|------|------|
| 日常 `./start_ollama.sh` | ✅ ~200 条/天 |
| R1 eval_p1_subset | ✅ |
| R2 patch 14 条 | ✅ |
| R3 验证批次 L1/L2 | ⏳ 明天更新 |
| O1–O3 规则开发 | ⬜ 待 R3 数据 + 错误池验证后立项 |
...
| 时间 | 动作 | 状态 |
|------|------|------|
| 07-08/09 | Track B 288 条 MLX | ✅ |
| 07-09~10 | E1–E5′ MLX v2/v3/v4 | ❌ smoke 2/5，暂停 MLX 灌库 |
| 07-13 | R1/R2 Ollama 规则轨 | ✅ |
| **07-15~** | **Ollama 测试运营 ~200 条/天** | ⏳ |
| **07-16** | R3 准确率更新 | ⬜ 待用户 |
| 后续 | O1–O3 规则迭代 + 周报告 | ⬜ |
