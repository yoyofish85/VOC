...
| R29 | 复核确认交互优化（A+B） | 行内「确认正确」按钮 + `⋯` 存疑/未复核；不触碰模型字段 | 🧪 已验证，待旧服部署 |
| R30 | 下一阶段路线图（硬件/L3/洞察） | 214 个定位 L3 → 30–45 个汇报主题；M5 Max 模型门禁；跨记录洞察 | 📋 计划中 |
...
### R29 · 复核确认交互优化 A+B（2026-09-12）

#### 背景

行级「人工复核」为 `el-select`（未复核/已复核/存疑），日常最高频动作「认可模型结果」需要点开下拉再移动鼠标选择；存疑/未复核几乎不用。

#### 方案

- **A（行内一键）**：`review_status !== 1` 时显示 `确认正确` 按钮；已复核显示 `已复核` 标签。低频的存疑/未复核收到 `⋯` 下拉内。
- **B（批量）**：工具栏新增 `确认所选正确 (n)`，走 `batchSaveReviewApi`。

确认语义：人工标签为空时**采纳模型 v3 结果**（`非问题` 不带 L2，业务三类缺 L2 则拒绝并提示），再置 `review_status=1`。

#### 变更文件

- `src/frontend/src/utils/reviewConfirm.js`（新增，纯逻辑）
- `src/frontend/src/utils/reviewConfirm.test.js`（新增，`node --test`，9/9 通过）
- `src/frontend/src/components/OpinionReview.vue`

#### 识别链路影响：无

`qwen_ollama.py` / 规则链 / `batch_classify` 未改；`/save_review`、`/batch_save_review` 仅写 `review_*`，**不写** `v3_label_meta` / `v3_l1`（单测已断言）。

#### 部署

沿用原打包脚本 `./code_deploy/package_code.sh`（内含 `npm run build`，打包全部 `src/`），无需改动。

### R30 · 下一阶段路线图：硬件 / L3 / 汇报洞察（2026-09-12，计划中）

详见 **`docs/plans/2026-09-12-l3-insight-hardware-roadmap.md`**。要点：

1. **硬件/模型**：Mac Studio M5 Max 128GB 解除「36GB 放不下 32B」约束。核心思路是**从「一个模型干所有活」改为「按任务分配模型」**——当前 14B 同时做分类、L2/L3 精排和汇报文案，但汇报是「一周几次、每次都要给人看」的任务，质量最敏感、延迟最不敏感，最该用大模型却一直和分类共用 14B。
   - 角色分配：分类主力 **Qwen3-32B q4**（~20GB，约 28 tok/s）；低置信仲裁 + **汇报洞察生成** 用 **Qwen3-72B q4**（~43GB）；新增 **`bge-m3` 嵌入模型**（~1.2GB）支撑跨 L2 语义聚集。三者常驻约 64GB，在 90–96GB 可用空间内
   - 需设 `OLLAMA_MAX_LOADED_MODELS=3`、`OLLAMA_KEEP_ALIVE=-1`（顺带解决历史冷启动问题）、`OLLAMA_NUM_PARALLEL=1`
   - 前置代码改动：分类与汇报共用 `VOC_QWEN_MODEL`，需新增 `VOC_QWEN_MODEL_REPORT` / `VOC_QWEN_MODEL_ARBITER`
   - 迁移分 M-1~M-6 六步、每步可独立回滚；**M-3（仅汇报切 72B）风险最低、收益最直接，不必等 M-5 换分类模型**
   - 换分类模型必须过 `ab_compare_models.py` holdout A/B + 近 7 天 dry-run 门禁（规则链是按 14B 输出调优的）
2. **L3 标签体系**（方案已按阶段 0 实测重写）：
   - **`review_l3` 全库 0 条**（已复核 8575）→ 不存在复核员填写的 L3 金标；"人工三级标签"实为标注体系文件 `label_project/label_hierarchy_final.json`（635 个去重 L3，带频次）
   - **聚类体系 492 个 canonical 中 488 个已在人工体系内，仅 4 个独有** → "融合两套标签"是伪命题；聚类只用于频次交叉校验与关键词串
   - 标准 L3 取人工体系**频次 ≥ 9 约 150 个，覆盖 79.6% 历史数据量**；须**按 L1 配额**（建议 75/30/40/5），全局 Top-150 会让体验需求类只剩 21 个
   - 生产 `v3_l3` Top15 除「通用」外 **14/14 命中人工体系** → 映射有效、无需重建；**「通用」占 2056/9126 = 22.5%**，是 ROI 最高的专项
   - 新发现结构问题：人工体系「体验需求类」有 **224 个 L2**（whitelist 只认 24 个），`L3/L2 = 1.08` 层级塌陷，152 个 L2 频次 ≤ 2 → 需「伪 L2 下沉」独立任务
3. **汇报洞察**：诉求是「透视更多信息，不是简单信息提取」。两者的分界线是**数据范围**——提取在单条记录内，透视必须**跨记录/跨时间/跨维度**。当前汇报之所以像信息罗列，根因是所有分析都停在「单条记录 + 按标签计数」这一层。
   - **第一层（单条结构化）**：`intent`（含「投诉升级」）/ `expected_action` / `severity` / `urgency` / `repeat_signal` / `root_cause_hint`，规则为主可离线重放
   - **第二层（跨记录透视，本方向本体）**：① 按 `vin`/`phone` 算首次解决率与未解决存量（直接回答「我们的处理到底有没有用」，现在完全答不了）；② `intent` 跨期迁移算情绪升级率（总量持平但升级占比上升 = 用户耐心耗尽）；③ 期望未满足率；④ 车型/区域集中度（从「有充电投诉」升级到「70% 来自某车型而该车型仅占 20%」）；⑤ 故障生命周期五态；⑥ 跨 L2 语义聚集；⑦ 时滞分析
   - **跨 L2 聚集的实证**：充电这一根因被标签体系切成闪充站问题 809 + 家充桩问题 335 + 无法充电 149…，按 L2 看是四个中等问题，按根因看是超 1300 条的头号问题
   - **`severity` 加权排序**是必要的，否则「希望增加 APP 功能」这类低危高频永远压过「行驶中动力中断」
   - **核心交付物是洞察模板库 I1–I10**：预设分析假设每期自动检验、命中即输出，保证每期必有透视结论而不依赖 LLM 灵感；每条洞察带 `evidence_sql` 可回溯
   - **LLM 只负责表达，不参与发现**：所有结论由确定性计算得出
   - **最小可交付切片**：R-0（维度盘点）→ R-1（意图字段）→ R-2（跨记录指标）→ R-5（只上 I1/I2/I6），**不依赖硬件升级、不依赖 L3 定版、不依赖嵌入模型**
   - **R-0 是前置**：`vin`/`phone`/`car_model` 填充率决定复发分析与车型集中度能不能做，命令见计划文档 §6.3
4. **阶段 0 盘点：✅ 已完成（2026-09-12）**，结果见上条与计划文档 §1.3b。

当前决策（2026-09-13）：先处理产品质量类与服务类，定版约 **214 个问题定位 L3**；再建立约 **30–45 个汇报主题**，每个定位 L3 必须唯一映射到一个计数用主主题，并可下钻至 L3 清单和原始记录。体验需求类暂缓；**L3 暂不进 KPI**；L1/L2 门禁口径不变。

**待办 T1**：✅ **2026-09-13 已到**（`VOC工作表_数据表 1.1_体验需求类.csv`）。范围按业务决定：**先做产品质量类 + 服务类，体验需求类暂缓**。入库报告见 `docs/plans/2026-09-13-l3-manual-intake.md`。产品质量 430 L3 / 服务类 85 L3；标准层草案 214 个；L2 白名单有 4 处差异未改。

**硬件 ETA**：新服务器 **8 周后到货（≈2026-11-07）**。过渡期 8 周执行计划见计划文档 §5，监督机制与周度进度表见 §5b。两个关键发现：

- **`bge-m3` 嵌入模型仅 ~1.2GB，可与 14B 在旧机 36GB 上共驻** → 方向二最费力的「跨 L2 语义聚集」**不必等新机**，8 周内可完整落地
- **固定 holdout 曾为空，已于 2026-09-12 补建完成**（302 条，14B baseline L1 90.07%）→ 后续 32B 可在旧机非生产时段尝试离线 A/B

**P0（本周，零依赖）**：
- ✅ **P0-1 完成（2026-09-12）**：vin 67.9%/phone 76.1%/car_model 68.5%；VIN 复发率 55.6%；「通用」2056 条中 **83% 为 generic_l3**，Top4 L2 占 99.1%（销售服务 640/售后 594/座舱 534/APP 269）→ L3-4b 优先补这 4 个 L2 的子标签
- ✅ **P0-2 完成（2026-09-12）**：holdout=302；baseline L1=**90.07%**（272/302）
- ⬜ P0-3 同步缺失工具脚本到服务器（§6.5）
- 🔄 P0-4 R29 已提交并推送（`1cebd4b`），待旧服务器部署验收

待决策：是否开启人工 L3 采集（建议仅对高频 L2 可选填）、标准 L3 的 L1 配额比例。
